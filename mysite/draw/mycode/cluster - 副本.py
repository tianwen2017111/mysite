...
def clustering_by_ip(G, ip_seg=2):
    ...
    if seg_No == 4:
        seg_group = set(std_ip)

    for vtx in std_ip:
        dot_pos = find_char_in_str(vtx, '.')    #点在ip地址中的位置
        dot_pos.append(len(vtx))
        s = vtx[:dot_pos[seg_No-1]]
